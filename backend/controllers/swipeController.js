...
// Get potential matches based on interests and skills, including their timeslot
...
// Swipe action, handling both left and right swipe (including timeslot in notification)
    ...
    try {
        ...
        if (isAccepted) {
            if (currentUser.matchRequests.includes(swipedUser._id)) {
                ...
                currentUser.notifications.push(`🎉 You matched with @${swipedUser.username}! Time slot: ${swipedUser.timeslot || 'Not Set'}`)
                swipedUser.notifications.push(`🎉 You matched with @${currentUser.username}! Time slot: ${currentUser.timeslot || 'Not Set'}`)
